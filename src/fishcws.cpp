...
private:
	iconv_t m_iconv;
public:
	CodeConverter(const string& inFormat , const string& outFormat ){
		m_iconv  = iconv_open(outFormat.c_str(),inFormat.c_str());
		if( m_iconv == NULL){
			fprintf(stderr,"iconv_open fail %s to %s",inFormat.c_str(),outFormat.c_str());
			exit(-1);
		}
	}
	string Convert(const string& in){
		string out;
		char* inBuffer = (char*)malloc(in.size()+1);
		size_t inBufferLeft = in.size();
		char* outBuffer = (char*)malloc(in.size()*4);
		size_t outBufferLeft = in.size()*4;
		char* inBufferOrigin = inBuffer;
		char* outBufferOrigin = outBuffer;

		memcpy(inBuffer,in.c_str(),in.size()+1);
		bzero(outBuffer,outBufferLeft);
...
extern string findLetter(int nCode);
class Pinyin{
public:
	struct WordInfo{
	public:
...
private:
	CodeConverter m_codeCoverter;
	CodeConverter m_codeCoverterReturn;
	string getLetterPinyin( int code );
public:
	Pinyin():m_codeCoverter("UTF-8","GB2312"),m_codeCoverterReturn("GB2312","UTF-8"){
	}
	vector<WordInfo> getPinyin(const string& inStrText){
		vector<WordInfo> result;
		//转换成GB2312编码
		string strText = m_codeCoverter.Convert(inStrText);
		//查表转换拼音
		for( size_t i = 0 ; i < strText.size() ; ++i ){
			WordInfo singleWordInfo;
			if ( (unsigned char)strText[i] < 0x80 ){
				//普通的ASCII字符
				if( isalnum(strText[i]) == false ){
					singleWordInfo.word = strText[i];
					singleWordInfo.pinyin = "";
				}else{
					for( ; i < strText.size() && isalnum(strText[i]) ; ++i ){
						singleWordInfo.word += strText[i];
						singleWordInfo.pinyin += strText[i];
					}
					i -= 1;
				}
			}else{
				...
				if( i + 1 < strText.size() )
					ucLow = (unsigned char)strText[++i];
				else
					ucLow = 0;
				if( ucHigh < 0xa1 || ucLow < 0xa1){
					singleWordInfo.word += ucHigh;
					singleWordInfo.word += ucLow;
					singleWordInfo.word = m_codeCoverterReturn.Convert(singleWordInfo.word);
					singleWordInfo.pinyin = "";
				}else{
					...
					singleWordInfo.word += ucLow;
					singleWordInfo.word = m_codeCoverterReturn.Convert(singleWordInfo.word);
					singleWordInfo.pinyin = getLetterPinyin(nCode);
					for( size_t j = 0 ; j < singleWordInfo.pinyin.size() ; ++j )
						singleWordInfo.pinyin[j] = tolower(singleWordInfo.pinyin[j]);
				}
			}
			result.push_back(singleWordInfo);
		}
		return result;
	}
	~Pinyin(){
...
public:
	...
	vector<WordInfo> ParagraphProcess( const string& input ){
		vector<WordInfo> result;
		int nCount = 0;
...
private:
	...
	Pinyin pinyin;
public:
	Business(const std::string& httpcws_settings_datapath):nlptr(httpcws_settings_datapath),pinyin(){
	}
	Json::Value Handle( const Json::Value& inputJson ){
		Json::Value outputValue;
		if( inputJson["type"].isString() && inputJson["type"] == "getSegment"
			&& inputJson["data"].isString()){
			//分词请求
			vector<NLPTR::WordInfo> output = nlptr.ParagraphProcess(inputJson["data"].asString());
			Json::Value vecWordInfo;
			for( unsigned int i = 0 ; i < output.size() ; ++i ){
				Json::Value singleWordInfo;
				singleWordInfo["word"] = output[i].word;
				singleWordInfo["pos"] = output[i].pos;
				singleWordInfo["weight"] = output[i].weight;
				vecWordInfo.append(singleWordInfo);
			}
			outputValue["code"] = 0;
			outputValue["msg"] = "";
			outputValue["data"] = vecWordInfo;
		}else if(inputJson["type"].isString() && inputJson["type"] == "getPinyin"
			&& inputJson["data"].isString()){
			//汉语转拼音请求
			vector<Pinyin::WordInfo> output = pinyin.getPinyin(inputJson["data"].asString());
			Json::Value vecWordInfo;
			for( unsigned int i = 0 ; i < output.size() ; ++i ){
				Json::Value singleWordInfo;
				singleWordInfo["word"] = output[i].word;
				singleWordInfo["pinyin"] = output[i].pinyin;
				vecWordInfo.append(singleWordInfo);
			}
			outputValue["code"] = 0;
			outputValue["msg"] = "";
...
string Pinyin::getLetterPinyin(int nCode)
{
	string strValue;
	switch(nCode)
	{
